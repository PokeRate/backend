...
def fetch_pokemon_moves_and_create(auth_token, offset):
    ...
    """
    pokeapi_url = f"https://pokeapi.co/api/v2/move/?limit=100000&offset={offset}"
    django_backend_url = "http://localhost:8000/api/pokemon/moves/"
    ...
    try:
        ...
        for info in data.get("results", []):
            ...
            pp = details["pp"]
            power = details["power"]
            type_details = requests.get(
                details["type"]["url"]).json()
